package de.voldechse.wintervillage.potterwars.utils.border;
...
public class WorldBorderController {
    ...
    public WorldBorderController() {
        ...
        if (this.borderPhases.isEmpty()) {
            Bukkit.getLogger().info("Could not find any border phases");
            return;
        }
        ...
        this.movingBorder.setPlugin(PotterWars.getInstance());
    ...
    public void start() {
        if (this.currentPhaseIndex >= 0 && this.currentPhaseIndex < this.borderPhases.size()) {
            ...
            Bukkit.getLogger().info("Start moving border from [oldRadius=" + currentPhase.oldSize + "] to [newRadius=" + currentPhase.newSize + "] for " + currentPhase.timeShrinking + " SECONDS [+" + currentPhase.pufferForNextBorder + " PUFFER]");
            Bukkit.broadcastMessage(PotterWars.getInstance().serverPrefix + "§eDie Barriere schrumpft");
    ...
    private void startBufferTimer(BorderPhase phase) {
        ...
        this.updatingTimerTask = new BukkitRunnable() {
            ...
            public void run() {
                ...
                time--;
                PotterWars.getInstance().scoreboardManager.updateScoreboard("currentPhase", " §fAktuelle Border§8:", "");
                PotterWars.getInstance().scoreboardManager.updateScoreboard("phaseSeconds", " §d" + String.format("%02d:%02d", (time / 60), (time % 60)), "");
                PotterWars.getInstance().scoreboardManager.updateScoreboard("phaseSize", " §a" + ((int) Math.round(world.getWorldBorder().getSize())) + "§8x§a" + ((int) Math.round(world.getWorldBorder().getSize())), " §4➜ §d" + phase.getNewSize() + "§8x§d" + phase.getNewSize());
            }
        }.runTaskTimer(PotterWars.getInstance(), 20L, 20L);

        this.announceNextPhaseTask = new BukkitRunnable() {
            @Override
            public void run() {
                Bukkit.broadcastMessage(PotterWars.getInstance().serverPrefix + "§eDie Barriere hat ihr §c" + (phase.id + 1) + ". §eZiel erreicht");
                PotterWars.getInstance().gameManager.playSound(Sound.BLOCK_NOTE_BLOCK_PLING, 1.0f);
                ...
                Bukkit.getLogger().info("Border reached it's destination [radius=" + phase.newSize + "]");

                if (isLast()) {
                    Bukkit.broadcastMessage(PotterWars.getInstance().serverPrefix + "§cDie Barriere hat ihr endgültiges Ziel erreicht");
                    PotterWars.getInstance().gameManager.playSound(Sound.BLOCK_NOTE_BLOCK_PLING, 1.0f);

                    PotterWars.getInstance().scoreboardManager.updateScoreboard("currentPhase", " §fNächste Border§8:", "");
                    PotterWars.getInstance().scoreboardManager.updateScoreboard("phaseSeconds", " §cEnde erreicht", "");
                    Bukkit.getLogger().info("Border reached it's final phases destination");
                    stopTasks();
                    cancel();
                    return;
                }
                ...
                Bukkit.getLogger().info("Next border phase will start in " + bufferTime + " SECONDS");
            }
        }.runTaskLater(PotterWars.getInstance(), timeShrinking * 20L);
        ...
        if (!isLast()) {
            ...
            }.runTaskLater(PotterWars.getInstance(), (timeShrinking + bufferTime) * 20L);
        }
    }
    ...
    private void startAnnouncingTimer(int var0) {
        ...
        this.announceTimerTask = new BukkitRunnable() {
            ...
            public void run() {
                ...
                PotterWars.getInstance().scoreboardManager.updateScoreboard("currentPhase", " §fNächste Border§8:", "");
                PotterWars.getInstance().scoreboardManager.updateScoreboard("phaseSeconds", " §d" + String.format("%02d:%02d", (puffer / 60), (puffer % 60)), "");
                PotterWars.getInstance().scoreboardManager.updateScoreboard("phaseSize", " §b" + borderSize + "§8x§b" + borderSize, "");
            }
        }.runTaskTimer(PotterWars.getInstance(), 20L, 20L);
    }
    ...
    private void loadFromConfig() {
        String world = PotterWars.getInstance().worldBorderDocument.getString("world");
        if (Bukkit.getWorld(world) == null) {
            Bukkit.getLogger().warning("Could not find " + world + " for border phases");
            this.world = Bukkit.getWorld("world");
        }

        this.world = Bukkit.getWorld(world);

        JsonArray controller = PotterWars.getInstance().worldBorderDocument.getArray("controller");
