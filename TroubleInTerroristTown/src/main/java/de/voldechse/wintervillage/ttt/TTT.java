...
@PlatformPlugin(
        ...
        commands = {
                @Command(name = "admin",
                        permission = "wintervillage.event.krampushunt.command.admin"),
                @Command(name = "build",
                        permission = "wintervillage.event.krampushunt.command.build"),
                @Command(name = "elf",
                        permission = "wintervillage.event.krampushunt.command.elf"),
                @Command(name = "krampus",
                        permission = "wintervillage.event.krampushunt.command.krampus"),
                @Command(name = "mrfrost",
                        permission = "wintervillage.event.krampushunt.command.mrfrost"),
                @Command(name = "pause",
...
public class TTT implements PlatformEntrypoint {
    ...
    @Inject
    public TTT(JavaPlugin plugin) {
        this.instance = plugin;

        this.instance.getLogger().info("-----------------------------------");
        this.instance.getLogger().info("TTT wird initialisiert!");
    ...
    public void onLoad() {
        this.instance.getLogger().info("-----------------------------------");
        this.instance.getLogger().info("config.json | tester.json | spawns.json | chests.json werden initialisiert...");
        this.configDocument = Document.loadDocument(Paths.get(this.instance.getDataFolder().getAbsolutePath(), "config.json"));
        this.testerDocument = Document.loadDocument(Paths.get(this.instance.getDataFolder().getAbsolutePath(), "tester.json"));
        this.spawnsDocument = Document.loadDocument(Paths.get(this.instance.getDataFolder().getAbsolutePath(), "spawns.json"));
        this.chestsDocument = Document.loadDocument(Paths.get(this.instance.getDataFolder().getAbsolutePath(), "chests.json"));
        ...
        this.roleManager.roleList.add(new Role(0, "Elf", "§a", "§fUnterstütze die Frosts dabei die Krampusse zu enttarnen", maxPercent_innocents, new ArrayList<Player>()));
        this.roleManager.roleList.add(new Role(1, "Mr. Frost", "§9", "§fIdentifiziere Leichen und enttarne die Krampusse", maxPercent_detectives, new ArrayList<Player>()));
        ...
        this.instance.getLogger().info("Es wurden folgende Teams erstellt");
        ...
        this.instance.getLogger().info("Items für das Spiel werden deklariert");
        ...
        this.instance.getLogger().info("Initializing protocol manager");
        protocolManager = ProtocolLibrary.getProtocolManager();
        this.initializePacketListening();

        this.instance.getLogger().info("TTT wurde initialisiert!");
        this.instance.getLogger().info("-----------------------------------");
    }
    ...
    public void onDisable() {
        this.instance.getLogger().info("-----------------------------------");
        this.instance.getLogger().info("TTT wird deaktiviert");
        ...
        this.instance.getLogger().info("TTT wurde deaktiviert");
        this.instance.getLogger().info("-----------------------------------");
    }

    private void initializePacketListening() {
        protocolManager.addPacketListener(new PacketAdapter(this.instance, ListenerPriority.NORMAL,
    ...
    private void registerListener() {
        ...
        try {
            for (ClassPath.ClassInfo classInfo : ClassPath.from(getClass().getClassLoader()).getTopLevelClasses("de.voldechse.wintervillage.ttt.listener")) {
    ...
    private void initializeTesterConfig_notAvailable() {
        if (Files.exists(Paths.get(this.instance.getDataFolder().getAbsolutePath(), "tester.json"))) return;

        this.instance.getLogger().severe("tester.json wurde nicht gefunden, weshalb sie nun erstellt wird");
        ...
        new Document("lampBlock_idling", "WHITE_STAINED_GLASS")
                ...
                .saveAsConfig(Paths.get(this.instance.getDataFolder().getAbsolutePath(), "tester.json"));

        this.instance.getLogger().info("tester.json wurde erstellt!");
    }

    private void initializeSpawnsConfig_notAvailable() {
        if (Files.exists(Paths.get(this.instance.getDataFolder().getAbsolutePath(), "spawns.json"))) return;

        this.instance.getLogger().severe("spawns.json wurde nicht gefunden, weshalb sie nun erstellt wird");
        ...
        new Document(UUID.randomUUID().toString(), new PositionEntity(
                ...
                0.0F,
                "world")
        ).saveAsConfig(Paths.get(this.instance.getDataFolder().getAbsolutePath(), "spawns.json"));

        this.instance.getLogger().info("spawns.json wurde erstellt!");
    }

    private void initializeChestsConfig_notAvailable() {
        if (Files.exists(Paths.get(this.instance.getDataFolder().getAbsolutePath(), "chests.json"))) return;

        this.instance.getLogger().severe("chests.json wurde nicht gefunden, weshalb sie nun erstellt wird");
        ...
        new Document(UUID.randomUUID().toString(), new PositionBlock(
                ...
                "world",
                "CHEST")
        ).saveAsConfig(Paths.get(this.instance.getDataFolder().getAbsolutePath(), "chests.json"));

        this.instance.getLogger().info("chests.json wurde erstellt!");
    }
    ...
    public CorpseEntity getCorpse(int entityId) {
        return this.CORPSES_MAP.get(entityId);
    }
