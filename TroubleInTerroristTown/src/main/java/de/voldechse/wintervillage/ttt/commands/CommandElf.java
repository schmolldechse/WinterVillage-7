...
public class CommandElf implements CommandExecutor {
    ...
    public CommandElf() {
        ...
        this.plugin.getInstance().getCommand("elf").setExecutor(this);
    }
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String label, String[] args) {
        ...
        if (this.plugin.roleManager.isPlayerAssigned(player)) {
            ...
            if (role.roleId == 0) {
                ...
                this.plugin.roleManager.removeFromRole(player, 0);
                return true;
            }

            this.plugin.roleManager.setCurrentRole(player, role.roleId, 0);
            player.sendMessage(this.plugin.serverPrefix + "§eDu wirst nun §f" + this.plugin.roleManager.getRole(0).getRolePrefix() + this.plugin.roleManager.getRole(0).getRoleName() + " §esein");
            return true;
        }

        this.plugin.roleManager.setCurrentRole(player, -1, 0);
        player.sendMessage(this.plugin.serverPrefix + "§eDu wirst nun §f" + this.plugin.roleManager.getRole(0).getRolePrefix() + this.plugin.roleManager.getRole(0).getRoleName() + " §esein");
        return false;
    }
}